...
var FileWhere = struct {
	...
	IsConfigured: whereHelperbool{field: `is_configured`},
...
var (
	fileColumns               = []string{"id", "path", "name", "content", "is_configured", "last_modified"}
...
func (fileL) LoadServices(ctx context.Context, e boil.ContextExecutor, singular bool, maybeFile interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(qm.From(`services`), qm.WhereIn(`file_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
